...
function update_address(hash, txid, amount, type, data, cb) {
    ...
    if (address) {
      ...
      if (type == 'vin') {
        ...
        var newAddress = new Address({
          ...
          unspent: [ {txid: txid, vout: data.n, scriptPubKey: data.scriptPubKey.hex, satoshis: (data.valueSat)?data.valueSat:data.value*100000000 } ],
          received: amount,
          balance: amount,
        });
      }
...
function remove_array_unspent( arr_unspent, cb ) {
    lib.syncLoop(arr_unspent.length, function (loop) {
        var n = loop.iteration();
        remove_from_unspent( arr_unspent[n].txid, arr_unspent[n].index , function(ret) {
            loop.next();
          });
      }, function(){
          return cb();
    ...
    if (tx != 'There was an error. Check your console.') {
      lib.get_block(tx.blockhash, function(block){
        if (block) {
          lib.prepare_vin(tx, function(vin) {
            lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) {
              lib.syncLoop(vin.length, function (loop) {
                ...
                update_address(nvin[i].addresses, txid, nvin[i].amount, 'vin', tx.vin[i] , function(){
                  loop.next();
                });
              }, function(){
                lib.syncLoop(vout.length, function (subloop) {
                  ...
                  if (vout[t].addresses) {
                    update_address(vout[t].addresses, txid, vout[t].amount, 'vout', tx.vout[t], function(){
                      subloop.next();
                    });
                  } else {
                    subloop.next();
                  ...
                  lib.calculate_total(vout, function(total){
                    ...
                    newTx.save(function(err) {
                      if (err) {
                        return cb(tx_unspent, err);
                      } else {
                        return cb(tx_unspent);
                      }
                    });
                  });
                });
          ...
          // UTXO (unspent) maintenance
          lib.syncLoop(tx.vin.length, function (vinloop) {
            var n = vinloop.iteration();
            if (!tx.vin[n].coinbase) {
              tx_unspent.push( {txid: tx.vin[n].txid, index: tx.vin[n].vout} );  
              vinloop.next();
            }
            else {
              vinloop.next();
            ...
            lib.syncLoop(tx.vout.length, function (voutloop) {
              var m = voutloop.iteration();
              if (tx.vout[m].scriptPubKey.addresses && tx.vout[m].scriptPubKey.addresses[0]) {
                add_to_unspent(tx.vout[m].scriptPubKey.addresses[0], txid, tx.vout[m].n, tx.vout[m], function(){
                  voutloop.next();
                });
              } else {
                  voutloop.next();
          ...
          return cb(tx_unspent, 'block not found: ' + tx.blockhash);
        }
      });
    } else {
      return cb(tx_unspent, 'tx not found: ' + txid);
    }
  });
}


function remove_from_unspent(txid, index, cb)
{
    Address.find({'unspent':{$elemMatch: {'txid':txid, 'vout':index}}}).exec(function(err, addresses){
      if (err || addresses.length!=1) {
        return cb(err);
      } else {
          var arr_unspent = addresses[0].unspent; 
          var removeIndex = arr_unspent.map(function(item) { return item.txid; }).indexOf(txid);      
          arr_unspent.splice(removeIndex, 1); 
		  Address.update({a_id:addresses[0].a_id}, {
            unspent: arr_unspent,
          }, function() {
            return cb();
          });
...
function add_to_unspent(hash, txid, index, unspent_data, cb)
{
    find_address(hash, function(address) {
      if (address) {
          var arr_unspent = address.unspent; 
          arr_unspent.push( {txid: txid, vout: unspent_data.n, scriptPubKey: unspent_data.scriptPubKey.hex, satoshis: (unspent_data.valueSat)?unspent_data.valueSat:unspent_data.value*100000000 } );
          Address.update({a_id:hash}, {
            unspent: arr_unspent,
          }, function() {
            return cb();
...
module.exports = {
  ...
  create_tx: function(txid, cb) {
    save_tx(txid, function(tx_unspent, err){
      if (err) {
        return cb(err);
      } else {
  ...
  create_txs: function(block, cb) {
    lib.syncLoop(block.tx.length, function (loop) {
      var block_unspent = [];
      var i = loop.iteration();
      save_tx(block.tx[i], function(tx_unspent, err){
          Array.prototype.push.apply(block_unspent,tx_unspent);
          loop.next();
      });
    }, function(){
      remove_array_unspent(block_unspent, function ()  {
        return cb();    
      });
    });
  },
